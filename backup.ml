    ...
    | hd::tl -> (match hd with 
        ...
        | And -> 
         (match y with 
              ...
              |_-> mainFunction tl y z) else if  List.length(temp) = 1 then (match temp with
              hd44::tl55 -> (match hd44 with
              B a ->  mainFunction tl (B(":true:")::t) z 
              |_-> mainFunction tl (S(":error:")::t) z
              )
              |_-> mainFunction tl (S(":error:")::t) z
              )  else (match y with
        h::h2::t -> (match h,h2 with 
        ...
        | N a, N b -> if checkDup b z then mainFunction tl (S(":unit:")::t)(overwriteFunction2 b z a) 
                      else if bindFunction2 z a = S(":error:") then mainFunction tl (S(":error:")::y) z
                      else mainFunction tl (S (":unit:")::t) ((b,(bindFunction2 z a))::z) 
        | P a, N b -> if a=":error:" then mainFunction tl (S(":error:")::y) z  
                      else if checkDup b z then mainFunction tl (S(":unit:")::t)(overwriteFunction b z hd)
        ...
        hd::hd2::hd3::t -> (match hd,hd2,hd3 with
        hd, hd2, B a -> if a = ":true:" then mainFunction tl (hd::t) z 
        else if a = ":false:" then mainFunction tl (hd2::t) z 
        else mainFunction tl (S(":error:")::y) z

        | N a, N b, N c -> if boolFunction z c = ":true:" then mainFunction tl ((bindFunction2 z a)::t) z
        else if (boolFunction z c) = ":false:" then mainFunction tl ((bindFunction2 z b)::t) z
        else mainFunction tl (S(":error:")::y) z

        | N a, hd2, N c -> if boolFunction z c = ":true:" then mainFunction tl ((bindFunction2 z a)::t) z 
...
interpreter ("input/sample_input.txt" ,"output.txt" );;